"""
Lineage & Impact Agent — Kafka I/O

Consumes:  agents.anomalies         → AnomalySignal
...
class AnomalyConsumer:
    """Consumes AnomalySignal messages from agents.anomalies."""

    def __init__(self, bootstrap_servers: str, group_id: str = "lineage-impact"):
        ...
        logger.info("lineage_consumer_subscribed | topic=%s", TOPIC_ANOMALIES)
...
class IncidentProducer:
    """Produces ConfirmedIncident and AgentHeartbeat messages."""
    ...
    def emit_incident(self, incident: ConfirmedIncident) -> None:
        ...
        logger.info(
            "incident_emitted | incident=%s severity=%s model=%s",
            incident.incident_id[:8],
            incident.severity.value,
            incident.correlated_signals[0].model_name if incident.correlated_signals else "unknown",
        )
